...
class CannotEmploy(Exception):
    """
    This exception will be raised when a `Extractor` is unable to employ a `Pop`. 
    This exception is expected to always be caught so that the pop can be redirected to another `Extractor` object 
    that may have available job positions.
    """
...
class Extractor:
    ...
    def employ(self, pop: Pop) -> Pop:
        """
        Checks for if the job is in the extractor's workforce labor_demand should be done from outside the method.
        """
        
        if pop.job and pop.job not in self.workforce.needed:
            raise ValueError(f'Pop with inappropriate job "{pop.job}" was passed to an `Extractor` object.')

        labor_demand = self.workforce.labor_demand()

        if pop.job:
            if pop.job not in labor_demand:
                return pop
